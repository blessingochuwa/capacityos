...
There is still no *hard* delete, and Phase 15's invariant guarantees an inactive organization always retains at least one active Owner able to reactivate it. Both actions are surfaced in the frontend on the Owner-only `/admin/organization` page (Phase 32) — a Deactivation section and an inactive-organization recovery panel — which perform no safety logic of their own and surface the backend's 422/403/404 verbatim. Phase 33 added the read-only `is_active` flag to the organization summary in `/auth/me`, so a deactivated *active* organization is announced app-wide by a persistent shell banner (with a recovery link for an Owner) and a deactivated organization is no longer offered as a normal choice in the organization switcher — all UX only; `get_current_membership` re-checking `is_active` per request stays the boundary.
...
### AI scenario-vs-baseline prioritization comparison explanation — interpretation only, never a second comparison engine

`POST /api/v1/ai/explain-scenario-priority-comparison` (`AIService.explain_scenario_priority_comparison`) explains an *existing* Phase 20 baseline-vs-scenario comparison — a seventh capability on the unchanged Phase 8 pipeline (`AIContextBuilder`/`AIService`/grounding), alongside `summarize`/`explain-signal`/`explain-scenario`/`explain-priority`/`explain-snapshot-comparison`. This is the item Phase 20's own brief explicitly named as intentionally deferred "for a future phase to consider deliberately." `AIContextBuilder.build_for_scenario_priority_comparison` calls `ScenarioPriorityService.compare` verbatim; no score, rank, or category is ever recomputed by the AI layer (CLAUDE.md §4/§21), and the cross-organization/unknown-id (404) behavior that call already enforces carries straight through to this route with no new authorization or tenancy code. A new `scenario_priority_comparison` grounding reference type adds one `(type, project_id)` pair per comparison item, matching the "one reference per collection member" shape `snapshot_comparison`/`signal`/`skill_coverage` already established. Gated by `Permission.AI_USE` only, granted to every role, exactly like every other AI endpoint — no new permission, no new grant-scoping. See [ADR 0026](adr/0026-ai-scenario-priority-comparison-explanation.md).

### Priority vs. Effort scatter — another of the PRD's own five §15 visualizations, zero backend changes

A scatter plot, per project scored under a RICE or WSJF framework, of its already-computed score (Y) against its effort-like criterion value (X) (`features/prioritization/utils/priorityEffortScatter.ts::buildPriorityEffortScatter`, `features/prioritization/components/PriorityEffortScatterChart.tsx`). Like Phase 25's WSJF Breakdown, this required no new backend code: `GET /api/v1/prioritization/portfolio` (unchanged since Phase 17) already returns every scored project's `score` and `breakdown`. RICE's effort criterion is literally named `"effort"`; WSJF's structurally analogous denominator is named `"job_size"` — the PRD's own §5.1 table already lists both formulas as value-divided-by-effort, so treating these as the same axis concept is grounded in existing code and the PRD's own table, not invented. `app/domain/prioritization.py::calculate_ice_score` confirms ICE has no effort-like denominator at all (`(Impact + Confidence + Ease) / 3`, a plain average never divided by anything), and Weighted Scoring's criteria are fully organization-defined with no reliable "effort" key — both are excluded rather than guessed at, the same "no fabricated interpretation" discipline WSJF Breakdown already established for MoSCoW. `buildPriorityEffortScatter` copies each value verbatim, never recomputing anything, and excludes any project without a complete score for the resolved effort criterion rather than plotting a fabricated zero. No quadrant lines or "quick win" threshold are drawn — no such boundary is defined anywhere in this codebase, and CLAUDE.md §17/§29's "no false precision"/"no misleading chart" rules apply exactly as they do everywhere else in this codebase. See [ADR 0027](adr/0027-priority-effort-scatter-visualization.md).

### What's still deferred

No rank-over-time or toggleable score/rank variant of the Phase 24 trend chart — audited and explicitly not selected (a MoSCoW score and rank are each always null, and rank conflates a project's own change with the portfolio around it). No Priority Explanation frontend panel beyond the single `ExplainPriorityButton`; the remaining three Recharts visualizations the PRD's own §15 names (Capacity-vs-Priority matrix, Risk-vs-Value quadrant, dependency timeline) are still unbuilt — a Phase 25 audit found Capacity-vs-Priority and Risk-vs-Value each carry real, unspecified cross-domain ambiguity (whose capacity over what period; which of a project's several risks), and the dependency timeline remains blocked (`ProjectDependency` still has no date/duration data, reconfirmed unchanged by the Phase 26 and Phase 27 audits). No Import/Export registration for `ProjectPriorityScore`, `ProjectDependency`, `ScenarioPriorityOverride`, or `PortfolioSnapshot` (deferred, matching Risk/Stakeholder's own Phase 13/14 precedent) — a Phase 22 audit of the actual import/export code confirmed the gap directly and found a further open question: neither Risk nor Stakeholder has a Person/Project-style natural identity key for CSV upsert-matching, reconfirmed still open by the Phase 25, Phase 26, and Phase 27 audits. No snapshot of a scenario's hypothetical (rather than baseline) ranking — audited for Phase 22 and re-confirmed by the Phase 23, Phase 24, Phase 25, Phase 26, and Phase 27 audits (directly against current code — `ScenarioService.delete` still hard-deletes) as still genuinely blocked on a product decision, not merely unbuilt: `Scenario` (unlike `PrioritizationFramework`) supports a real, non-soft delete, so what happens to a scenario snapshot when its scenario is deleted needs deciding before this is buildable. No membership/user-management frontend — re-confirmed fully backend-ready with zero frontend surface by the Phase 22, Phase 25, Phase 26, and Phase 27 audits, but not selected for any of them (a materially larger multi-flow vertical slice than a single-capability slice). Each is a named, deliberately scoped boundary confirmed with the user before implementation — see the PRD's "Recommended v1 slice," ADR 0017's Consequences, ADR 0018's Consequences, ADR 0019's Consequences, ADR 0020's Consequences, ADR 0021's Consequences, ADR 0022's Consequences, ADR 0023's Consequences, ADR 0024's Consequences, ADR 0025's Consequences, ADR 0026's Consequences, and ADR 0027's Consequences for the full list and reasoning.
