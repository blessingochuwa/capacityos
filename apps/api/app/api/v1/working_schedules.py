...
def create_working_schedule(
    ...
    schedule = service.create(membership.organization_id, data)
...
def list_working_schedules(
    ...
    return [
        WorkingScheduleRead.model_validate(item)
        for item in service.list_for_person(membership.organization_id, person_id)
    ]
...
def get_working_schedule(
    ...
    return WorkingScheduleRead.model_validate(service.get(membership.organization_id, schedule_id))
...
def update_working_schedule(
    ...
    schedule = service.update(membership.organization_id, schedule_id, data)
    audit_service.record(
        actor_user_id=current_user.id,
        actor_email=current_user.email,
...
def delete_working_schedule(
    ...
    service.delete(membership.organization_id, schedule_id)
    audit_service.record(
        actor_user_id=current_user.id,
        actor_email=current_user.email,
