...
class AccessGrantService:
    """Instance-level resource authorization (Phase 11) — the one place a
    Manager's Team/Project write authority is checked against an explicit
    grant, and the one place grants are created/removed/listed. See
    ...
    def enforce_team_access(
        ...
        *,
        ...
        granted = self.team_grants.exists(user.id, team_id)
        if has_permission(user.role, permission, resource=ResourceScope(granted=granted)):
            return
    ...
    def enforce_project_access(
        ...
        *,
        ...
        granted = self.project_grants.exists(user.id, project_id)
        if has_permission(user.role, permission, resource=ResourceScope(granted=granted)):
            return
        audit_service.record(
            actor_user_id=user.id,
    ...
    def grant_team_access(
        self, team_id: uuid.UUID, user_id: uuid.UUID, *, granted_by: User
    ) -> TeamAccessGrant:
        if self.teams.get(team_id) is None:
            raise NotFoundError("Team", team_id)
        if self.users.get(user_id) is None:
            raise NotFoundError("User", user_id)
        if self.team_grants.exists(user_id, team_id):
            raise ConflictError("This user already has access to this team.")
        grant = TeamAccessGrant(
            user_id=user_id, team_id=team_id, granted_by_user_id=granted_by.id
        )
        return self.team_grants.add(grant)

    def revoke_team_access(self, team_id: uuid.UUID, user_id: uuid.UUID) -> None:
        grant = self.team_grants.get_by_user_and_team(user_id, team_id)
        if grant is None:
            raise NotFoundError("TeamAccessGrant", f"user={user_id} team={team_id}")
        self.team_grants.delete(grant)

    def list_team_grants(self, team_id: uuid.UUID) -> list[TeamAccessGrant]:
        if self.teams.get(team_id) is None:
            raise NotFoundError("Team", team_id)
        return self.team_grants.list_for_team(team_id)

    def grant_project_access(
        self, project_id: uuid.UUID, user_id: uuid.UUID, *, granted_by: User
    ) -> ProjectAccessGrant:
        if self.projects.get(project_id) is None:
            raise NotFoundError("Project", project_id)
        if self.users.get(user_id) is None:
            raise NotFoundError("User", user_id)
        if self.project_grants.exists(user_id, project_id):
            raise ConflictError("This user already has access to this project.")
        grant = ProjectAccessGrant(
            user_id=user_id, project_id=project_id, granted_by_user_id=granted_by.id
        )
        return self.project_grants.add(grant)

    def revoke_project_access(self, project_id: uuid.UUID, user_id: uuid.UUID) -> None:
        grant = self.project_grants.get_by_user_and_project(user_id, project_id)
        if grant is None:
            raise NotFoundError("ProjectAccessGrant", f"user={user_id} project={project_id}")
        self.project_grants.delete(grant)

    def list_project_grants(self, project_id: uuid.UUID) -> list[ProjectAccessGrant]:
        if self.projects.get(project_id) is None:
            raise NotFoundError("Project", project_id)
        return self.project_grants.list_for_project(project_id)

    # --- current-user accessible-ids (feeds /auth/me for the frontend) ----------

    def accessible_team_ids(self, user_id: uuid.UUID) -> list[uuid.UUID]:
        return [grant.team_id for grant in self.team_grants.list_for_user(user_id)]

    def accessible_project_ids(self, user_id: uuid.UUID) -> list[uuid.UUID]:
        return [grant.project_id for grant in self.project_grants.list_for_user(user_id)]
