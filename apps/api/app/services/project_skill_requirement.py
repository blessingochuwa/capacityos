...
class ProjectSkillRequirementService:
    """Organization-scoped (Phase 12) — project_id/skill_id are guaranteed
    ...
    def add(
        self, organization_id: uuid.UUID, project_id: uuid.UUID, data: ProjectSkillRequirementCreate
        ...
        if (
            self.repository.get_by_project_and_skill(project_id, data.skill_id, organization_id)
            is not None
        ):
    ...
    def update(
        ...
        requirement = self._get_owned(organization_id, project_id, requirement_id)
    ...
    def remove(
        self, organization_id: uuid.UUID, project_id: uuid.UUID, requirement_id: uuid.UUID
    ) -> None:
        requirement = self._get_owned(organization_id, project_id, requirement_id)
    ...
    def _get_owned(
        ...
        requirement = self.repository.get(requirement_id, organization_id)
