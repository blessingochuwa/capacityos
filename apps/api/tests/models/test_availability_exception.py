...
def test_create_fully_unavailable_exception(
    ...
    exception = make_availability_exception(db_session, organization=organization, person=person)

    assert exception.hours is None
    assert exception.availability_type is AvailabilityType.ANNUAL_LEAVE


def test_create_partially_available_exception(
    db_session: Session, organization: Organization
) -> None:
...
def test_negative_hours_is_rejected(db_session: Session, organization: Organization) -> None:
    person = make_person(db_session, organization=organization)
    with pytest.raises(IntegrityError):
        make_availability_exception(
            db_session, organization=organization, person=person, hours=Decimal("-1")
        )


def test_availability_type_has_no_db_check_constraint(
    db_session: Session, organization: Organization
) -> None:
