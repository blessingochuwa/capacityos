...
def test_create_valid_five_day_schedule(db_session: Session, organization: Organization) -> None:
    person = make_person(db_session, organization=organization)
    schedule = make_working_schedule(db_session, organization=organization, person=person)
...
def test_schedule_supports_uneven_days(db_session: Session, organization: Organization) -> None:
    ...
    schedule = make_working_schedule(
        db_session, organization=organization, person=person, entries=entries
    )
...
def test_duplicate_weekday_in_same_schedule_is_rejected(
...
def test_hours_above_24_is_rejected(db_session: Session, organization: Organization) -> None:
    person = make_person(db_session, organization=organization)
...
def test_deleting_schedule_cascades_to_entries(
    db_session: Session, organization: Organization
) -> None:
    person = make_person(db_session, organization=organization)
    schedule = make_working_schedule(db_session, organization=organization, person=person)
