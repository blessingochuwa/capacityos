...
from tests.factories import make_team, make_team_access_grant, make_user


def test_create_team_access_grant(db_session: Session) -> None:
    user = make_user(db_session)
    team = make_team(db_session)
    grant = make_team_access_grant(db_session, user=user, team=team)
...
def test_grant_records_who_granted_it(db_session: Session) -> None:
    ...
    team = make_team(db_session)
    grant = make_team_access_grant(db_session, user=manager, team=team, granted_by=admin)

    assert grant.granted_by_user_id == admin.id


def test_duplicate_team_access_grant_is_rejected(db_session: Session) -> None:
    user = make_user(db_session)
    team = make_team(db_session)
    make_team_access_grant(db_session, user=user, team=team)

    with pytest.raises(IntegrityError):
        make_team_access_grant(db_session, user=user, team=team)


def test_same_user_can_be_granted_multiple_teams(db_session: Session) -> None:
    user = make_user(db_session)
    design = make_team(db_session, name="Design")
    engineering = make_team(db_session, name="Engineering")
    make_team_access_grant(db_session, user=user, team=design)
    make_team_access_grant(db_session, user=user, team=engineering)
...
def test_deleting_user_cascades_to_team_access_grant_with_fk_enforcement() -> None:
    ...
    make_team_access_grant(session, user=user, team=team)

    session.execute(delete(User).where(User.id == user.id))
    session.flush()

    assert TeamAccessGrantRepository(session).list_for_team(team.id) == []
...
def test_deleting_team_cascades_to_team_access_grant_with_fk_enforcement() -> None:
    ...
    assert TeamAccessGrantRepository(session).list_for_user(user.id) == []


def test_repository_exists_and_get_by_user_and_team(db_session: Session) -> None:
    user = make_user(db_session)
    granted_team = make_team(db_session, name="Design")
    ungranted_team = make_team(db_session, name="Engineering")
    make_team_access_grant(db_session, user=user, team=granted_team)

    repository = TeamAccessGrantRepository(db_session)
    assert repository.exists(user.id, granted_team.id) is True
    assert repository.exists(user.id, ungranted_team.id) is False
    assert repository.get_by_user_and_team(user.id, granted_team.id) is not None
    assert repository.get_by_user_and_team(user.id, ungranted_team.id) is None
