...
def test_create_valid_allocation(db_session: Session, organization: Organization) -> None:
    ...
    allocation = make_allocation(
        db_session, organization=organization, person=person, project=project
    )
...
def test_negative_allocation_hours_is_rejected(
    db_session: Session, organization: Organization
) -> None:
    person = make_person(db_session, organization=organization)
    project = make_project(db_session, organization=organization)
    with pytest.raises(IntegrityError):
        make_allocation(
            ...
            person=person,
            project=project,
            allocation_hours=Decimal("-1"),
        )


def test_zero_allocation_hours_is_valid(db_session: Session, organization: Organization) -> None:
    person = make_person(db_session, organization=organization)
    project = make_project(db_session, organization=organization)
    allocation = make_allocation(
        db_session,
        ...
        person=person,
        project=project,
        allocation_hours=Decimal("0"),
    )
    assert allocation.allocation_hours == Decimal("0")


def test_deleting_project_cascades_to_allocations(
    db_session: Session, organization: Organization
) -> None:
    person = make_person(db_session, organization=organization)
    project = make_project(db_session, organization=organization)
    make_allocation(db_session, organization=organization, person=person, project=project)
